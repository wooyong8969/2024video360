...
# 모니터 해상도 관련 변수들
monitor = get_monitors()[0]  # 1번 모니터 해상도
monitor_width = monitor.width
monitor_height = monitor.height

display_shape = [monitor_height, monitor_width]
...
usafov = USAFoV(display_shape=display_shape,
...
while True:
    ...
    if right_eye_points and left_eye_points:
        eye_center = np.mean(right_eye_points, axis=0) if preferred_eye == 1 else np.mean(left_eye_points, axis=0)  # 주시안의 평균좌표 사용
