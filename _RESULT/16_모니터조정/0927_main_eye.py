...
# 모니터의 네 모서리 좌표들 (2개의 모니터)
display_corners = [
    cp.array([[-19.5, 19, 41 + z], [19.5, 19, 41 + z],
              [-19.5, 19, 14 + z], [19.5, 19, 14 + z]]),  # 1번
    cp.array([[-19.5, 19, 14 + z], [19.5, 19, 14 + z],
              [-19.5, 19, 0 + z], [19.5, 19, 0 + z]])   # 2번
]
...
display_shapes = [(monitor1.height, monitor1.width), (monitor2.height, monitor2.width)]
...
def adjust_monitor(key, corners):
    move_step = cp.float32(0.5)  # 이동량 CuPy 타입으로 설정
    monitor_changed = False  # 좌표가 변경되었는지 여부
